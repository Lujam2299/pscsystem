...
<x-livewire.monitoreo-layout :breadcrumb-items="[
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
        ...
        <div class="border border-gray-200 rounded-lg md:col-span-1 bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
            <div class="p-4 border-b border-gray-300 dark:border-gray-600">
                <div class="flex items-center justify-between w-full min-h-8">
                    <h2 class="text-base font-semibold text-gray-900 dark:text-white">Detalles de la Misión</h2>
                    <div class="text-sm text-gray-600 dark:text-gray-400">
                        <span class="inline-block px-3 py-1 text-base font-semibold text-white bg-gray-800 rounded-full shadow dark:bg-white/80 dark:text-gray-900">
                            {{ $mision->id }}
                        </span>
                    </div>
                </div>
            ...
            <div class="p-4">
                <div class="space-y-3">
                    <div>
                        ...
                        <p class="text-gray-900 dark:text-white">{{ $mision->nombre_clave ?? 'No definido' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-gray-900 dark:text-white">{{ $mision->fecha_inicio ? \Carbon\Carbon::parse($mision->fecha_inicio)->format('d/m/Y') : 'No definida' }}</p>
                    </div>
                    <div>
                        <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300">Fecha Fin:</h3>
                        <p class="text-gray-900 dark:text-white">{{ $mision->fecha_fin ? \Carbon\Carbon::parse($mision->fecha_fin)->format('d/m/Y') : 'No definida' }}</p>
                    </div>
                    <div>
                        <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300">Tipo de Servicio:</h3>
                    ...
                    <!-- Opcional: Mostrar detalles de hotel/aeropuerto si están en JSON -->
                    @if($mision->datos_hotel)
                        ...
                        <div>
                            ...
                            <p class="text-gray-900 dark:text-white">{{ $hotel['nombre'] ?? 'No definido' }}</p>
                        </div>
                    @endif
                    @if($mision->datos_aeropuerto)
                        @php $aeropuerto = json_decode($mision->datos_aeropuerto, true); @endphp
                        <div>
                            <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300">Aeropuerto:</h3>
                            <p class="text-gray-900 dark:text-white">{{ $aeropuerto['nombre'] ?? 'No definido' }}</p>
                        </div>
                    @endif
                    <!-- Opcional: Mostrar agentes -->
                    <!--
                    @if($agentes)
                        <div>
                            <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300">Agentes Asignados:</h3>
        ...
        <div class="border border-gray-200 rounded-lg md:col-span-2 bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
            <div class="relative">
                <div class="flex flex-wrap items-center justify-between w-full p-4 border-b border-gray-300 dark:border-gray-600 min-h-8">
            ...
            <div class="p-4">
                <div id="mapaDetalleContainer" class="w-full bg-gray-200 rounded h-96 dark:bg-gray-700"></div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            const estadoMapaDetalle = { inicializado: false };

            // Función para crear una geocerca
            ...
            function inicializarMapaDetalle() {
                if (estadoMapaDetalle.inicializado) return; // Evitar re-inicializar
                ...
                mapaDetalle = L.map(contenedor).setView([25.6866, -100.3161], 13); // Coordenadas por defecto
                ...
                // Cargar las geocercas recibidas desde el servidor
            ...
            function cargarGeocercasDetalle(geocercasData) {
                ...
                if (grupoGeocercasDetalle.getLayers().length > 0) {
                    try {
                        ...
                        mapaDetalle.fitBounds(tempGroup.getBounds(), { padding: [50, 50] });
                    } catch (e) {
