            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                ...
                @if($yaRegistrado)
                    <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-6 rounded-r">
                        <p><strong>Advertencia:</strong> Ya se registró asistencia para el punto <strong>{{ $punto }}</strong> hoy. Este registro reemplazará el anterior.</p>
                    </div>
                @endif
                ...
                <form action="{{ route('operaciones.guardarAsistencias') }}" method="POST" enctype="multipart/form-data" id="form-asistencias">
                    @csrf
                    ...
                    @if($elementos->isEmpty())
                        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6 text-center mb-6">
                            <p class="text-blue-800 dark:text-blue-200">No hay personal activo asignado al punto <strong>{{ $punto }}</strong>.</p>
                        </div>
                    @else
