{{-- resources/views/livewire/historial-deducciones.blade.php --}}
@php
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    Consulte el historial completo de deducciones registradas
                </p>
            </div>
    ...
    <!-- Controles de Filtro en una sola fila -->
    <div class="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="sm:col-span-2 lg:col-span-1">
            ...
            <div class="relative">
                ...
                <input type="text"
                       wire:model.live.debounce.300ms="buscarUsuario"
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fecha Inicio Desde</label>
            <input type="date"
        ...
        <!-- Filtro por Fecha Inicio Hasta -->
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fecha Inicio Hasta</label>
            <input type="date"
    ...
    <!-- Mensaje de Carga -->
    <div wire:loading class="mb-4 flex items-center text-sm text-blue-600 dark:text-blue-400">
        <svg class="animate-spin -ml-1 mr-2 h-4 w-4 text-blue-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Filtrando deducciones...
    </div>
    ...
    @if($deducciones->isEmpty())
        <div class="text-center py-12">
            ...
            <p class="text-gray-500 dark:text-gray-400 mb-6">No se encontraron registros de deducciones con los filtros aplicados.</p>
        </div>
    @else
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($deducciones as $deduccion)
                            @if(
                                ...
                                str_contains(strtoupper($deduccion->user->rol ?? ''), 'ESCOLTA')
                            )
                            @else
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">
                                            @if($deduccion->user)
                                                {{ $deduccion->user->name }}
                                            @elseif($deduccion->user_id && $deduccion->user()->withTrashed()->first())
                                                {{ $deduccion->user()->withTrashed()->first()->name }}
                                            ...
                                            @else
                                                <span class="text-gray-500 dark:text-gray-400">Usuario no encontrado</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                        {{ Str::limit($deduccion->concepto, 30) }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                        ${{ number_format($deduccion->monto, 2) }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                        ${{ number_format($deduccion->monto_pendiente ?? 0, 2) }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                        {{ $deduccion->num_quincenas }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                        {{ $deduccion->fecha_inicio?->format('d/m/Y') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                            {{ $deduccion->status === 'Pagado' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-200' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-200' }}">
                                            {{ $deduccion->status }}
                                        </span>
                                    </td>
                                </tr>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-6 py-12 whitespace-nowrap text-center">
                                    ...
                                    <p class="text-gray-500 dark:text-gray-400">No se encontraron deducciones con los filtros aplicados.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-center gap-3">
            ...
            {{-- Opcional: Agrega aquí botones como "Exportar Excel", "Generar Reporte", etc. si los necesitas --}}
        </div>
    </div>
</div>
